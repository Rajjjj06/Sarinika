 ...
 * This runs client-side where we have authentication context
 */
export async function checkNotifications(userId: string): Promise<string[]> {
  try {
    ...
    // 4. Check for first entry welcome notification
    const welcomeNotification = await checkWelcomeNotification(userId)
    if (welcomeNotification) {
      createdNotifications.push(welcomeNotification)
    }

    console.log(`✅ Created ${createdNotifications.length} notification(s)`, createdNotifications)
...
async function checkStreakNotifications(
  ...
  if (preferences.milestones.enabled && streak > 0) {
    ...
    for (const milestone of milestones) {
      if (streak === milestone) {
        // Check if we already notified about this milestone (simplified query)
...
async function checkMoodNotifications(
  ...
  if (avgMood <= preferences.moodAlerts.threshold) {
    ...
    // Check if we already sent a mood alert recently (simplified query to avoid index)
    ...
    // Filter in memory for recent alerts (within last 24 hours)
    const recentAlerts = existingAlerts.docs.filter((doc) => {
      const data = doc.data()
      if (data.createdAt?.toDate) {
        return data.createdAt.toDate() >= yesterday
      }
      return false
    })
  ...
  if (recentJournals.length >= 3) {
    ...
    if (older3.length >= 3) {
      ...
      if (recentAvg > olderAvg + 1) {
        ...
        // Check if we already sent improvement notification (simplified query)
        ...
        // Filter in memory for recent notifications (within last 24 hours)
        const recentNotifications = existing.docs.filter((doc) => {
          const data = doc.data()
          if (data.createdAt?.toDate) {
            return data.createdAt.toDate() >= yesterday
          }
...
/**
 * Check for first entry and create welcome notification
 */
async function checkWelcomeNotification(userId: string): Promise<string | null> {
  try {
    // Check if user has any notifications already
    ...
    // If user already has notifications, skip welcome
    if (!existingNotifications.empty) {
      return null
    }

    // Check if this is their first journal entry
    const journalsQuery = query(
      collection(db, "journalEntries"),
      where("userId", "==", userId)
    )
    ...
    // Only show welcome if they have exactly 1 entry (just created)
    if (journalsSnapshot.size === 1) {
      const notificationRef = await addDoc(collection(db, "notifications"), {
        userId,
        type: "encouragement",
        title: "Welcome to Your Journey! 🌟",
        message: "Great job on your first journal entry! Keep writing to unlock achievements and track your progress.",
        read: false,
        actionUrl: "/dashboard",
        actionLabel: "View Dashboard",
      ...
      console.log("✅ Created welcome notification:", notificationRef.id)
